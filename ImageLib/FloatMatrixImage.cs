...
//        /// </summary>
//        /// <param name="m"></param>
//        /// <param name="func">i, j, src[i,j], dst[i,j]</param>
//        /// <returns></returns>
...
//        public static Matrix<float> Mediane(Matrix<float> m, int size, Func<float[,], float> func)
//        {
//            if (size % 2 == 0) throw new ArgumentException($"{nameof(size)}%2!=0", nameof(size));
//            int cols = m.ColumnCount;
//            int rows = m.RowCount;
//            float[,] src = m.ToArray();
//            float[,] dst = m.ToArray();
...
//                        {
//                            vs[ii, jj] = src[i + ii, j + jj];
//                        }
//                    }
//                    dst[i + d, j + d] = func(vs);
//                }
//            }
//            return Matrix<float>.Build.DenseOfArray(dst);
//        }

//        public static Matrix<float> Mediane(Matrix<float> m, int kcols, int krows, Func<float[,], float> func)
//        {
//            if (kcols % 2 == 0) throw new ArgumentException($"{nameof(kcols)}%2!=0", nameof(kcols));
//            if (krows % 2 == 0) throw new ArgumentException($"{nameof(krows)}%2!=0", nameof(krows));
//            int cols = m.ColumnCount;
//            int rows = m.RowCount;
//            float[,] src = m.ToArray();
//            float[,] dst = m.ToArray();
//            float[,] vs = new float[kcols, krows];
//            int w = cols - kcols;
//            int h = rows - krows;
//            int dcols = (kcols - 1) / 2;
//            int drows = (krows - 1) / 2;
...
//        public static Bitmap CreateBitmap(float[,] vs)
//        {
//            Bitmap bitmap = new Bitmap(vs.GetLength(1), vs.GetLength(0), PixelFormat.Format8bppIndexed);
//            var palette = bitmap.Palette;
//            var ent = palette.Entries;
//            for (int i = 0; i < 256; i++)
//            {
...
//        public static void FillBitmap(Bitmap bitmap, byte[,] vs)
//        {
//            BitmapData bitmapData = bitmap.LockBits(new Rectangle(Point.Empty, bitmap.Size),
//                                                  ImageLockMode.WriteOnly,
//                                                  PixelFormat.Format8bppIndexed);
...
//            int stride = bitmapData.Stride;
//            int bytes = Math.Abs(stride) * bitmap.Height;
//            int cols = vs.GetLength(0);
//            int rows = vs.GetLength(1);
//            byte[] vs1 = new byte[Math.Abs(stride) * bitmap.Height];
//            for (int i = 0; i < cols; i++)
//            {
//                int v = i * stride;
//                for (int j = 0; j < rows; j++)
...
//                }
//            }
//            System.Runtime.InteropServices.Marshal.Copy(vs1, 0, intPtr, bytes);
//            bitmap.UnlockBits(bitmapData);
//        }
